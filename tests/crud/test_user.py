...
def test_create_user(db: Session) -> None:
    ...
    user_in = UserCreate(full_name="User", email=email, password=password)
...
def test_check_if_user_is_inactive(db: Session) -> None:
    ...
    password = random_lower_string()
    user_in = UserCreate(
        full_name="User", email=email, password=password, is_active=False
    )
    user = crud_user.user.create(db, obj_in=user_in)
    is_active = crud_user.user.is_active(user)
...
def test_check_if_user_is_admin(db: Session) -> None:
    ...
    password = random_lower_string()
    user_in = UserCreate(
        full_name="User", email=email, password=password, is_admin=True
    )
    user = crud_user.user.create(db, obj_in=user_in)
    is_admin = crud_user.user.is_admin(user)
...
def test_check_if_user_is_superuser_normal_user(db: Session) -> None:
    ...
    password = random_lower_string()
    user_in = UserCreate(full_name="User", email=username, password=password)
    user = crud_user.user.create(db, obj_in=user_in)
    is_superuser = crud_user.user.is_superuser(user)
